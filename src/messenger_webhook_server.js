...
import { money, trailerPolicyText, buildBundleQuote, multiDayQuote, applyTrailerToQuote } from "./pricing.js";
import {
  normalize, containsAny, parseDays, isMonthlyRequest, isPriceQuestion, isWeightQuestion,
  isThumbQuestion, isBucketOrCabQuestion, bookingIntent, isDeliveryQuestion, isTrailerQuestion,
  isDeliveryPriceQuestion, isTrailerIncludedQuestion, wantsTrailerAddedToTotal, wantsDeliveryAddedToTotal,
  deliveryInfo, arrangedBoomLiftIntent, isMulcherQuestion, isMulcherComboQuestion, isMulcherOnlyQuestion,
  isReferentialFollowup, findEquipment, findAllEquipment, findCategory, hasExplicitIntentOverride
} from "./intent.js";
...
function getLastMulcherComboChoice(state) {
  if (state.lastMulcherComboChoice === "cat" || state.lastMulcherComboChoice === "jd") return state.lastMulcherComboChoice;
...
function mentionsCatSide(text) {
  return text === "cat" || text.includes("the cat") || containsAny(text, ["cat combo", "cat mulcher combo", "caterpillar", "cat 265", "hm316", "cat mulcher"]);
}

function mentionsJdSide(text) {
  return text === "john deere" || text === "jd" || text.includes("the john deere") || text.includes("the jd") || containsAny(text, ["john deere combo", "john deere mulcher combo", "jd combo", "333p", "mh60d", "jd mulcher", "john deere mulcher"]);
}
...
function detectMulcherComboChoice(text, state) {
  ...
  if (lastCombo === "cat" && containsAny(t, ["what about the jd", "what about jd", "what about john deere", "tell me about the jd", "tell me about john deere"])) return "jd";
  if (lastCombo === "jd" && containsAny(t, ["what about the cat", "what about cat", "tell me about the cat", "tell me about cat"])) return "cat";
  return null;
}
...
function singleQuote(item, id) {
  ...
  if (item.category === "scissor_lift") lines.push("These are slab scissor lifts, not rough-terrain scissor lifts. Rough-terrain scissor lifts must be special ordered.");
...
if (
  ...
  if (isMulcherQuestion(message)) {
    ...
    if (isMulcherOnlyQuestion(message)) {
      return preserveContext(state, { text: `We have a CAT HM316 Forestry Mulcher (${money(EQUIPMENT[ITEM_IDS.CAT_MULCHER].day)}/day) and a John Deere MH60D Forestry Mulcher (${money(EQUIPMENT[ITEM_IDS.JD_MULCHER].day)}/day). The CAT HM316 is usually better for longer rentals since the carbide teeth don’t need sharpening.`, lastId: null, lastCategory: "mulcher", lastCategoryItems: [ITEM_IDS.CAT_MULCHER, ITEM_IDS.JD_MULCHER], lastQuotedItems: [] });
    }
    return preserveContext(state, { text: "Do you need the mulcher and skid steer or just the mulcher?", lastId: null, lastCategory: "mulcher", lastCategoryItems: [ITEM_IDS.CAT_MULCHER, ITEM_IDS.JD_MULCHER], lastQuotedItems: [] });
  }
  ...
  if (state.lastCategory === "mulcher_combo" && text === "both") {
    return preserveContext(state, { text: "Which combo do you need — CAT HM316 + CAT 265 or John Deere MH60D + John Deere 333P? The Boxer and CAT 239 cannot be used with either mulcher.", lastId: null, lastCategory: "mulcher_combo", lastCategoryItems: [ITEM_IDS.CAT_MULCHER, ITEM_IDS.CAT_265, ITEM_IDS.JD_MULCHER, ITEM_IDS.JD_333P] });
  }
  ...
  if (isDeliveryQuestion(message) && !isPriceQuestion(message) && !wantsDeliveryTotal) {
    if (isDeliveryPriceQuestion(message)) {
      if (delivery) return preserveContext(state, { text: `Delivery for ${delivery.placeLabel} is ${money(delivery.fee)}.`, lastCategory: null, lastCategoryItems: [], lastDeliveryFee: delivery.fee, lastDeliveryPlace: delivery.placeLabel });
      return preserveContext(state, { text: "What city or area are you in? Delivery pricing depends on location.", lastId: item ? id : state.lastId, lastCategory: "delivery_followup", lastCategoryItems: [], lastQuotedItems: item ? [id] : state.lastQuotedItems });
    }
    if (delivery) return preserveContext(state, { text: `Yes, we can deliver there. Delivery for ${delivery.placeLabel} is ${money(delivery.fee)}.`, lastCategory: null, lastCategoryItems: [], lastDeliveryFee: delivery.fee, lastDeliveryPlace: delivery.placeLabel });
    return preserveContext(state, { text: `We deliver within about a ${DELIVERY_RADIUS_MILES}-mile radius. What city or area are you in? Delivery pricing depends on location.`, lastId: item ? id : state.lastId, lastCategory: "delivery_followup", lastCategoryItems: [], lastQuotedItems: item ? [id] : state.lastQuotedItems });
  }
  ...
  if (isTrailerQuestion(message)) {
    ...
    if (isTrailerIncludedQuestion(message)) return preserveContext(state, { text: `No, the trailer does not come with it automatically. ${trailerPolicyText(requestedDays)}` });
    if (containsAny(text, ["weight requirements", "what trailer do i need", "what size trailer", "how heavy is it"])) return preserveContext(state, { text: trailerWeightText(state) });
  ...
  if (category === "scissor_lift" && !explicitFound) {
    return preserveContext(state, { text: `We have two slab scissor lifts available:\n\n• Genie GS1930 – ${money(EQUIPMENT[ITEM_IDS.GS1930].day)}/day, ${money(EQUIPMENT[ITEM_IDS.GS1930].week)}/week, ${money(EQUIPMENT[ITEM_IDS.GS1930].month)}/month\n• Genie GS3246 – ${money(EQUIPMENT[ITEM_IDS.GS3246].day)}/day, ${money(EQUIPMENT[ITEM_IDS.GS3246].week)}/week, ${money(EQUIPMENT[ITEM_IDS.GS3246].month)}/month\n\nThese are slab scissor lifts, not rough-terrain scissor lifts. Rough-terrain scissor lifts must be special ordered.`, lastId: null, lastCategory: "scissor_lift", lastCategoryItems: CATEGORY_ITEMS.scissor_lift });
  }
  if (category === "boom_lift" && !explicitFound) return preserveContext(state, { text: `We have a Genie Z45 Articulating Boom Lift (${money(EQUIPMENT[ITEM_IDS.GENIE_Z45].day)}/day) and a JLG ET500J Towable 50' Boom Lift. For current ET500J pricing, please check the website at ${WEBSITE}. If you need a larger, taller, or specialty boom lift, we can arrange one. Please schedule online at ${WEBSITE} or call/text Dave at ${DAVE_PHONE}.`, lastId: null, lastCategory: "boom_lift", lastCategoryItems: CATEGORY_ITEMS.boom_lift });
  if (category === "excavator" && !explicitFound) return preserveContext(state, { text: `We have ${formatCategoryQuote(CATEGORY_ITEMS.excavator)}.`, lastId: null, lastCategory: "excavator", lastCategoryItems: CATEGORY_ITEMS.excavator });
  if (category === "skid_steer" && !explicitFound) return preserveContext(state, { text: `We have ${formatCategoryQuote(CATEGORY_ITEMS.skid_steer)}.`, lastId: null, lastCategory: "skid_steer", lastCategoryItems: CATEGORY_ITEMS.skid_steer });
  if (category === "telehandler" && !explicitFound) return preserveContext(state, { text: `We have a JLG 6K Telehandler for ${money(EQUIPMENT[ITEM_IDS.TELEHANDLER].day)}/day and ${money(EQUIPMENT[ITEM_IDS.TELEHANDLER].week)}/week. Monthly pricing is quoted by Dave based on current market conditions.`, lastId: ITEM_IDS.TELEHANDLER, lastCategory: null, lastCategoryItems: [], lastQuotedItems: [ITEM_IDS.TELEHANDLER], lastQuote: buildBundleQuote([ITEM_IDS.TELEHANDLER], 1, 0) });
  if (category === "forklift" && !explicitFound) return preserveContext(state, { text: "Yes—we have a standard forklift, a rough-terrain forklift, and a telehandler. Are you looking for a warehouse-style forklift, rough-ground forklift, or a lull?", lastId: null, lastCategory: "forklift", lastCategoryItems: [ITEM_IDS.FORKLIFT, ITEM_IDS.LIFT_KING, ITEM_IDS.TELEHANDLER] });
  if (category === "pressure_washer" && !explicitFound) return preserveContext(state, { text: `Yes, we have a pressure washer for ${money(EQUIPMENT[ITEM_IDS.PRESSURE_WASHER].day)} a day. We also have a surface cleaner for ${money(EQUIPMENT[ITEM_IDS.SURFACE_CLEANER].day)} a day for flatwork.`, lastId: ITEM_IDS.PRESSURE_WASHER, lastCategory: null, lastCategoryItems: [], lastQuotedItems: [ITEM_IDS.PRESSURE_WASHER], lastQuote: buildBundleQuote([ITEM_IDS.PRESSURE_WASHER], 1, 0) });
  if ((category === "boom_lift" || state.lastCategory === "boom_lift" || item?.category === "boom_lift") && arrangedBoomLiftIntent(message)) return preserveContext(state, { text: `We can arrange larger or specialty boom lifts if needed. Specialty equipment is handled separately, so please schedule online at ${WEBSITE} or call/text Dave at ${DAVE_PHONE}.`, lastCategory: "boom_lift", lastCategoryItems: CATEGORY_ITEMS.boom_lift });
  if (bookingIntent(message) && !item && state.lastCategoryItems?.length > 1) return preserveContext(state, { text: categoryDisambiguationText(state.lastCategoryItems, "want to schedule") });
  ...
  if (item) {
    ...
    if (isMonthlyRequest(message) && id === ITEM_IDS.TELEHANDLER) return clearCategoryFields({ text: `Monthly pricing for ${item.name} is quoted by Dave based on current market conditions. Please call or text Dave at ${DAVE_PHONE}.`, lastId: id, lastQuotedItems: [id], lastQuote: state.lastQuote, lastMulcherComboChoice: state.lastMulcherComboChoice, lastDeliveryFee: state.lastDeliveryFee, lastDeliveryPlace: state.lastDeliveryPlace });
    if (isWeightQuestion(message)) return clearCategoryFields({ text: item.weight ? `${item.name} weighs ${item.weight.toLocaleString()} lb.` : (item.details || singleQuote(item, id)), lastId: id, lastQuotedItems: [id], lastQuote: state.lastQuote, lastMulcherComboChoice: state.lastMulcherComboChoice, lastDeliveryFee: state.lastDeliveryFee, lastDeliveryPlace: state.lastDeliveryPlace });
